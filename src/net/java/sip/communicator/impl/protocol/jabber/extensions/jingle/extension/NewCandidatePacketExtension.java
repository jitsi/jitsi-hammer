 ...
 * (e.g. change the normal CandidateType class in ice4j to list the candidate type enums
 * in preferred order, or implement a comparator)
 */
public class NewCandidatePacketExtension
        extends NewAbstractExtensionElement
        implements Comparable<NewCandidatePacketExtension>
{
    /**
     ...
     */
    public NewCandidatePacketExtension()
    {
        super(ELEMENT_NAME, null);
    }

    /**
     * Creates a new {@link NewCandidatePacketExtension} with the specified
     ...
     */
    protected NewCandidatePacketExtension(String elementName)
    {
     ...
     * NewCandidateType} enum.
     *
     * @param type this candidates' type as per ICE's RFC 5245.
     */
    public void setType(NewCandidateType type)
    {
     ...
     */
    public NewCandidateType getType()
    {
        return NewCandidateType.valueOf(getAttributeAsString(TYPE_ATTR_NAME));
    }

    /**
     * Compares this instance with another NewCandidatePacketExtension by
     * preference of type: host < local < prflx < srflx < stun < relay.
     *
     * @return 0 if the type are equal. -1 if this instance type is preferred.
     * Otherwise 1.
     */
    public int compareTo(NewCandidatePacketExtension newCandidatePacketExtension)
    {
        // If the types are different.
        if(this.getType() != newCandidatePacketExtension.getType())
        {
            NewCandidateType[] types = {
                    NewCandidateType.host,
                    NewCandidateType.local,
                    NewCandidateType.prflx,
                    NewCandidateType.srflx,
                    NewCandidateType.stun,
                    NewCandidateType.relay
            };
            for(int i = 0; i < types.length; ++i)
            {
                ...
                // the newCandidatePacketExtension is preferred.
                else if(types[i] == newCandidatePacketExtension.getType())
                {
                    return 1;
                }
            }
    ...
    /**
     * Gets the TCP type for this <tt>NewCandidatePacketExtension</tt>.
     */
    ...
    /**
     * Sets the TCP type for this <tt>NewCandidatePacketExtension</tt>.
     * @param tcpType
     */
